 ...
 * @implements {adapt.layout.BreakPosition}
 */
adapt.layout.EdgeBreakPosition = function(position, breakOnEdge, overflows, computedHeight) {
	/** @const */ this.position = position;
	/** @const */ this.breakOnEdge = breakOnEdge;
	/** @const */ this.overflows = overflows;
	/** @const */ this.computedHeight = computedHeight;
};
 ...
 * @param {number} footnodeEdgeOffset
 */
adapt.layout.FootnoteItem = function(boxOffset, startPosition, overflowPosition, footnodeEdgeOffset) {
	/** @const */ this.boxOffset = boxOffset;
	/** @const */ this.startPosition = startPosition;
	/** @const */ this.overflowPosition = overflowPosition;
	/** @const */ this.footnodeEdgeOffset = footnodeEdgeOffset;
};
 ...
 * @private
 */
adapt.layout.Column.prototype.makeNodeContext = function(step) {
	var nodeContext = new adapt.vtree.NodeContext(step.node, null, 0);
	nodeContext.shadowType = step.shadowType;
	nodeContext.shadowContext = step.shadowContext,
	nodeContext.nodeShadow = step.nodeShadow;
	nodeContext.shadowSibling = step.shadowSibling ? this.makeNodeContext(step.shadowSibling) : null;
	return nodeContext;
};
...
adapt.layout.Column.prototype.openAllViews = function(position) {
	...
	for (var stepIndex = 0; stepIndex < steps.length; stepIndex++) {
		...
		// TODO: create containers top-to-bottom to transfer properties from parent correctly
		var nodeContext = self.makeNodeContext(step);
		if (path.length == 0) {
			nodeContext.offsetInNode = position.offsetInNode;
			nodeContext.after = position.after;
		}
		if (prevContext) {
			prevContext.parent = nodeContext;
			while ((prevContext = prevContext.shadowSibling) != null) {
				prevContext.parent = nodeContext.copy();
			}
		}
		prevContext = nodeContext;		
    	path.push(nodeContext);
    };
    ...
    frame.loop(function() {
	    while (i >= 0) {
	    	...
	    	if (nc.after)
	    		break;
	    	var r = self.layoutContext.setCurrent(nc, i == 0 && nc.offsetInNode == 0);
	    	i--;
	    	if (r.isPending())
	    		return r;
	    }
 ...
 * @param {number} width float width.
...
adapt.layout.Column.prototype.calculateEdge = function(checkPoints, index, boxOffset) {
    ...
    while(true) {
        var edge = adapt.layout.calculateEdge(nodePosition, this.clientLayout, offset);
        if (!isNaN(edge))
            return edge;
        if (offset > 0) {
            offset--;
...
adapt.layout.Column.prototype.processFullyOverflownFootnote = function(boxOffset, footnoteNodePosition) {
	...
	var footnoteItem = new adapt.layout.FootnoteItem(boxOffset, footnoteNodePosition,
			footnoteNodePosition, this.footnoteEdge - this.beforeEdge);
 ...
 */
adapt.layout.Column.prototype.layoutFootnoteInner = function(boxOffset, footnoteNodePosition, minY) {
	var self = this;
	...
	minY += 40; // Avoid having it too high.
    ...
    if (firstFootnoteInColumn) {
    	...
    	self.layoutContext.applyFootnoteStyle(footnoteContainer);
	...
	var top = minY - self.beforeEdge;
	footnoteArea.width = self.box.x2 - self.box.x1
		- footnoteArea.getInsetLeft() - footnoteArea.getInsetRight();
	var height = self.afterEdge - minY - footnoteArea.getInsetTop() - footnoteArea.getInsetBottom();
	if (firstFootnoteInColumn && height < 18) {
		self.element.removeChild(footnoteArea.element);
		self.footnoteArea = null;
		self.processFullyOverflownFootnote(boxOffset, footnoteNodePosition);
		return adapt.task.newResult(true);
	}
	if (footnoteArea.top < top) {
		self.element.removeChild(footnoteArea.element);
		self.processFullyOverflownFootnote(boxOffset, footnoteNodePosition);
		return adapt.task.newResult(true);
	}
    ...
    footnoteArea.setVerticalPosition(top, height);
    ...
    initResult.then(function() {
		footnoteArea.layout(footnotePosition).then(function(footnoteOverflowParam) {
			...
			self.footnoteEdge = self.afterEdge - footnoteArea.computedHeight
				- footnoteArea.getInsetTop() - footnoteArea.getInsetBottom();
			var footnoteEdgeOffset = self.footnoteEdge - self.beforeEdge;
			footnoteArea.setVerticalPosition(footnoteEdgeOffset, footnoteArea.computedHeight);
			var redoResult;
			if (footnoteArea.exclusions.length > 0) {
			...
			redoResult.then(function(footnoteOverflow) {
				...
				var footnoteItem = new adapt.layout.FootnoteItem(boxOffset, footnoteNodePosition,
						overflowPosition, footnoteEdgeOffset);
				if (self.footnoteItems) {
...
adapt.layout.Column.prototype.layoutFootnote = function(nodeContext) {
    ...
    var callBoxBottom = callBox.bottom;
    element.textContent = "";
    ...
    self.layoutFootnoteInner(boxOffset, footnoteNodePosition, callBoxBottom).then(function() {
	    if (self.footnoteArea && self.footnoteArea.element.parentNode) {
	    	self.element.removeChild(self.footnoteArea.element);
	    }
	    if (callBoxBottom < self.footnoteEdge || self.breakPositions.length == 0) {
 ...
 * @param {adapt.vtree.NodeContext} nodeContext
 * @return @return {!adapt.task.Result.<adapt.vtree.NodeContext>}
 */
adapt.layout.Column.prototype.layoutFloat = function(nodeContext) {
    ...
    self.buildDeepElementView(nodeContext).then(function(nodeContextAfter) {
	    ...
	    if (parent) {
	    	...
	    	x1 = Math.max(parentBox.left, x1);
	    	x2 = Math.min(parentBox.right, x2);
	    	parent.viewNode.removeChild(probe);        
	    	if (floatSide == "left")
	    		x2 = Math.max(x2, x1 + floatBox.x2 - floatBox.x1);
	    	else
	    		x1 = Math.min(x1, x2 - (floatBox.x2 - floatBox.x1));
	    }
	    var box = new adapt.geom.Rect(x1, self.box.y1, x2, self.box.y2);
	    adapt.geom.positionFloat(box, self.bands, floatBox, floatSide);
	    ...
	    if (floatBox.y2 <= self.footnoteEdge || self.breakPositions.length == 0) {
	    	self.killFloats();
	        adapt.geom.addFloatToBands(self.box, self.bands, floatBox, null, floatSide);
	        self.createFloats();
	        // no overflow
	    	self.updateMaxReachedAfterEdge(floatBox.y2);
...
adapt.layout.Column.prototype.processLineStyling = function(nodeContext, resNodeContext, checkPoints) {
	...
	frame.loopWithFrame(function(loopFrame) {
		...
		self.finishBreak(lineBreak, false);
...
adapt.layout.Column.prototype.layoutBreakableBlock = function(nodeContext) {
    ...
    self.buildViewToNextBlockEdge(nodeContext, checkPoints).then(function(resNodeContext) {
	    ...
	    var overflown = edge > self.footnoteEdge;
...
adapt.layout.Column.prototype.findLinePositions = function(checkPoints) {
    ...
    boxes.sort(adapt.vtree.clientrectIncreasingTop);
    var lineTop = 0;
    var lineBottom = 0;
    var lineEnd = 0;
    var lineLength = 0;
    ...
    while (true) {
    	if (i < boxes.length) {
	        ...
	        var overlap = 1;
	        if (lineLength > 0) {
	        	var height = Math.max(box.bottom - box.top, 1);
	        	if (box.top < lineTop) {
	        		overlap = (box.bottom - lineTop) / height;
	        	} else if (box.bottom > lineBottom) {
	        		overlap = (lineBottom - box.top) / height;
	        	} else {
	        		overlap = 1;
	        	}
	        }
        	if (lineLength == 0 || overlap >= MID_OVERLAP || 
        			(overlap >= LOW_OVERLAP && box.left >= lineEnd-1)) {
            	lineEnd = box.right;
	        	lineTop = lineLength == 0 ? box.top : Math.min(lineTop, box.top);
	        	lineBottom = lineLength == 0 ? box.bottom : Math.max(lineBottom, box.bottom);
	        	lineLength++;
	        	i++;
	        	continue;
        	}
...
adapt.layout.Column.prototype.clearFootnotes = function(boxOffset) {
	...
	var maxY = self.computedHeight + self.beforeEdge;
...
adapt.layout.Column.prototype.findBoxBreakPosition = function(bp, force) {
	...
	if (nodeContext) {
		this.computedHeight = edge - this.beforeEdge;		
	}
	return nodeContext;
};
...
adapt.layout.Column.prototype.findEdgeBreakPosition = function(bp) {
	this.computedHeight = bp.computedHeight;
	return bp.position;
};
...
adapt.layout.Column.prototype.findAcceptableBreak = function() {	
	...
	if (!nodeContext) {
		...
		this.finishBreak(nodeContext, true);
	}
    return nodeContext;
};
...
adapt.layout.Column.prototype.saveEdgeAndCheckForOverflow = function(nodeContext, saveEvenOverflown, breakAtTheEdge) {
    ...
    var edge = adapt.layout.calculateEdge(nodeContext, this.clientLayout, 0);
	var overflown = edge > this.footnoteEdge;
	this.updateMaxReachedAfterEdge(edge);
...
adapt.layout.Column.prototype.initGeom = function() {
    ...
    this.box = new adapt.geom.Rect(offsetX, offsetY, offsetX + this.width,
    		offsetY + this.height);
    this.beforeEdge = columnBBox ? columnBBox.top : 0;
    this.afterEdge = columnBBox ? columnBBox.bottom : 0;
    this.footnoteEdge = this.afterEdge;
    this.bands = adapt.geom.shapesToBands(this.box, [this.getInnerShape()],
    		this.exclusions, 8, this.snapHeight, false);
	this.createFloats();
...
adapt.layout.Column.prototype.saveEdgeBreakPosition = function(position, breakAtEdge, overflows) {
	var bp = new adapt.layout.EdgeBreakPosition(position.copy(), breakAtEdge, overflows, this.computedHeight);
...
adapt.layout.Column.prototype.updateMaxReachedAfterEdge = function(afterEdge) {
	this.computedHeight = Math.max(afterEdge - this.beforeEdge, this.computedHeight);
};
	...
	self.layoutOverflownFootnotes(chunkPosition).then(function() {
	    ...
	    self.openAllViews(chunkPosition.primary).then(function(nodeContext) {
			...
			frame.loopWithFrame(function(loopFrame) {
			    while (nodeContext) {
			        ...
			        self.layoutNext(nodeContext).then(function(nodeContextParam) {
						...
						if (self.pageBreakType) {
				        	...
				        	nodeContext = self.findAcceptableBreak();
				            loopFrame.breakLoop(); // Loop end
				        } else {
				        	if (pending) {
				        		// Sync case
			    ...
			    if (footnoteArea) {
			    	self.element.appendChild(footnoteArea.element);
			    	self.computedHeight = footnoteArea.top + footnoteArea.getInsetTop() +
			    		footnoteArea.computedHeight + footnoteArea.getInsetBottom();
			    }
...
/**
 * Re-layout already layed-out chunks. Return the position of the last flow if there is
 * an overflow.
