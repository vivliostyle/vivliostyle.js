...
/**
 * What a context accumulates while a page is laid out. Halving a footnote until
 * it fits (Issue #1879) and invalidating a line-policy footnote at most once
 * (Issue #2024, #2026) terminate only because this record outlives the
 * containers a retry rebuilds.
 */
type PageFloatContextState = {
  invalidated: boolean;
  locked: boolean;
  readonly children: AttachedPageFloatLayoutContext[];
  readonly floatAnchors: { [key in PageFloatID]: Node };
  readonly forbiddenFloats: PageFloatID[];
  readonly floatFragments: PageFloatFragment[];
  readonly stashedFloatFragments: PageFloatFragment[];
  readonly floatsDeferredToNext: PageFloatContinuation[];
  readonly floatsDeferredFromPrevious: PageFloatContinuation[];
  readonly layoutConstraints: LayoutType.LayoutConstraint[];
  /**
   ...
   */
  readonly footnoteAnchorsSeen: Set<PageFloatID>;
  /**
   ...
   * (Issue #2024, #2026)
   */
  readonly lineFootnoteInvalidationMaxBlockSize: Map<
    PageFloatID,
    number | null
  >;
  /**
   ...
   */
  readonly lineFootnoteFinishInvalidatedOnce: Set<PageFloatID>;
};

/** Unchanged by attachment, which is why identity-keyed walks ignore it. */
type PageFloatContextIdentity = {
  readonly parent: ParentPageFloatLayoutContext | null;
  readonly floatReference: FloatReference | null;
  readonly flowName: string | null;
  readonly generatingNodePosition: Vtree.NodePosition | null;
  readonly writingMode: Css.Val;
  readonly direction: Css.Val;
  readonly pageSide: Constants.PageSide | null;
  readonly outerContext: AttachedPageFloatLayoutContext | null;
  readonly floatStore: PageFloatStore;
};
 ...
 *     example page floats)
 */
export abstract class PageFloatLayoutContext
  implements PageFloats.PageFloatLayoutContext
{
  readonly parent: ParentPageFloatLayoutContext | null;
  protected readonly floatReference: FloatReference | null;
  readonly flowName: string | null;
  readonly generatingNodePosition: Vtree.NodePosition | null;
  readonly writingMode: Css.Val;
  readonly direction: Css.Val;
   ...
   */
  protected readonly outerContext: AttachedPageFloatLayoutContext | null;
  protected readonly floatStore: PageFloatStore;
  protected readonly state: PageFloatContextState;

  protected constructor(
    identity: PageFloatContextIdentity,
    state?: PageFloatContextState,
  ) {
    this.parent = identity.parent;
    this.floatReference = identity.floatReference;
    this.flowName = identity.flowName;
    this.generatingNodePosition = identity.generatingNodePosition;
    this.writingMode = identity.writingMode;
    this.direction = identity.direction;
    this.pageSide = identity.pageSide;
    this.outerContext = identity.outerContext;
    this.floatStore = identity.floatStore;
    this.state = state ?? this.newState();
  }

  protected static buildIdentity(
    parent: ParentPageFloatLayoutContext | null,
    ...
    outerContext: AttachedPageFloatLayoutContext | null,
    pageSide: Constants.PageSide | null,
  ): PageFloatContextIdentity {
    return {
      parent,
      floatReference,
      flowName,
      ...
      direction:
        ...
        parent?.direction ||
        Css.ident.ltr,
      pageSide: pageSide ?? parent?.pageSide ?? outerContext?.pageSide ?? null,
      outerContext,
      floatStore: parent
        ? parent.floatStore
  ...
  private newState(): PageFloatContextState {
    // This context is not in its parent's children yet.
    ...
    return {
      ...
      forbiddenFloats: [],
      floatFragments: [],
      stashedFloatFragments: [],
      floatsDeferredToNext: [],
      floatsDeferredFromPrevious: previousSibling
        ? [...previousSibling.state.floatsDeferredToNext]
        : [],
      layoutConstraints: [],
      footnoteMaxBlockSize: null,
      ignoreFootnoteAreaMaxHeight: false,
      footnoteAnchorsSeen: new Set(),
      lineFootnoteInvalidationMaxBlockSize: new Map(),
      lineFootnoteFinishInvalidatedOnce: new Set(),
    };
  }

  /**
   * A page float area takes no part in invalidation or exclusions, and reaches
   * its surroundings through its column instead. (Issue #1675)
   */
  static createDetached(
    outerContext: AttachedPageFloatLayoutContext,
    floatReference: FloatReference | null,
    flowName: string | null,
    generatingNodePosition: Vtree.NodePosition | null,
  ): UnattachedPageFloatLayoutContext {
    return new UnattachedPageFloatLayoutContext(
      PageFloatLayoutContext.buildIdentity(
        null,
        floatReference,
        flowName,
        generatingNodePosition,
  ...
  static create(
    parent: ParentPageFloatLayoutContext,
    floatReference: FloatReference | null,
    flowName: string | null,
    generatingNodePosition: Vtree.NodePosition | null,
    writingMode: Css.Val | null,
    direction: Css.Val | null,
    outerContext: AttachedPageFloatLayoutContext | null = null,
    pageSide: Constants.PageSide | null = null,
  ): UnattachedPageFloatLayoutContext {
    const identity = PageFloatLayoutContext.buildIdentity(
      parent,
      floatReference,
      flowName,
      generatingNodePosition,
    ...
    return new UnattachedPageFloatLayoutContext(identity);
  }
  ...
  static createWithContainer(
    ...
    return new AttachedPageFloatLayoutContext(identity, container);
  }

  protected static register(
    parent: ParentPageFloatLayoutContext | null,
    next: AttachedPageFloatLayoutContext,
  ): void {
    if (!parent) {
      return;
    }
    // One record per logical context. The child holding this one is what a
    // previous attempt attached.
    const index = parent.state.children.findIndex(
      (child) => child.state === next.state,
    );
    if (index >= 0) {
      parent.state.children[index] = next;
    } else {
      parent.state.children.push(next);
    }
  }

  protected identity(): PageFloatContextIdentity {
    return {
      parent: this.parent,
      floatReference: this.floatReference,
      flowName: this.flowName,
      generatingNodePosition: this.generatingNodePosition,
      writingMode: this.writingMode,
      direction: this.direction,
      pageSide: this.pageSide,
      outerContext: this.outerContext,
      floatStore: this.floatStore,
    };
  }

  get floatFragments(): PageFloatFragment[] {
    return this.state.floatFragments;
  }

  get footnoteMaxBlockSize(): number | null {
    return this.state.footnoteMaxBlockSize;
  }

  set footnoteMaxBlockSize(value: number | null) {
    this.state.footnoteMaxBlockSize = value;
  }

  get ignoreFootnoteAreaMaxHeight(): boolean {
    return this.state.ignoreFootnoteAreaMaxHeight;
  }

  set ignoreFootnoteAreaMaxHeight(value: boolean) {
    this.state.ignoreFootnoteAreaMaxHeight = value;
  }
  ...
  protected getParent(floatReference: FloatReference): PageFloatLayoutContext {
    if (this.parent) {
      return this.parent;
    }
  ...
  protected getPreviousSibling(): AttachedPageFloatLayoutContext | null {
    let child: PageFloatLayoutContext | null = this;
  ...
  isForbidden(float: PageFloat): boolean {
    ...
    if (floatReference === this.floatReference) {
      return this.state.forbiddenFloats.includes(id);
    } else {
  ...
  markPageFloatAnchorSeen(float: PageFloat) {
    ...
    while (ctx) {
      ctx.state.footnoteAnchorsSeen.add(float.getId());
      ctx = ctx.parent ?? ctx.outerContext;
    }
  }

  registerPageFloatAnchor(float: PageFloat, anchorViewNode: Node) {
    this.state.floatAnchors[float.getId()] = anchorViewNode;
  ...
  hasInvalidatedForLineFootnote(float: PageFloat): boolean {
    if (float.floatReference === this.floatReference) {
      return (
        this.state.lineFootnoteInvalidationMaxBlockSize.get(float.getId()) ===
  ...
  markInvalidatedForLineFootnote(float: PageFloat): void {
    if (float.floatReference === this.floatReference) {
      this.state.lineFootnoteInvalidationMaxBlockSize.set(
        float.getId(),
        this.footnoteMaxBlockSize,
      );
    } else {
  ...
  deferPageFloat(continuation: PageFloatContinuation) {
    ...
    if (float.floatReference === this.floatReference) {
      const index = this.state.floatsDeferredToNext.findIndex(
        (c) => c.float === float,
      );
      if (index >= 0) {
        this.state.floatsDeferredToNext.splice(index, 1, continuation);
      } else {
        this.state.floatsDeferredToNext.push(continuation);
      }
    } else {
      const parent = this.getParent(float.floatReference);
  ...
  removeFloatDeferredToNext(float: PageFloat) {
    ...
    if (index >= 0) {
      this.state.floatsDeferredToNext.splice(index, 1);
    }
  }
  ...
  hasPrecedingFloatsDeferredToNext(
    ...
    const hasPrecedingFloatsDeferredToNext =
      this.state.floatsDeferredToNext.some(
  ...
  getDeferredPageFloatContinuations(
    ...
    let result = this.state.floatsDeferredFromPrevious.filter(
  ...
  getPageFloatContinuationsDeferredToNext(
    ...
    flowName = flowName || this.flowName;
    const result = this.state.floatsDeferredToNext.filter(
      (cont) => !flowName || cont.float.flowName === flowName,
    );
    if (this.parent) {
      return this.parent
  ...
  protected trySetFootnoteRetryMaxBlockSize(currentOuter: number): boolean {
    const newMax =
  ...
  isInvalidated() {
    return (
      this.state.invalidated || (!!this.parent && this.parent.isInvalidated())
    );
  }
  ...
  protected toLogical(side: string): string {
    return toLogicalSide(side, this);
  }

  protected toPhysical(side: string): string {
    return toPhysicalSide(side, this);
  }

  protected toLogicalFloatSides(floatSide: string): string[] {
  ...
  discardStashedFragments(floatReference: FloatReference) {
    ...
    this.state.stashedFloatFragments.splice(0);
  }
  ...
  getLayoutConstraints(): LayoutType.LayoutConstraint[] {
    ...
    return constraints.concat(this.state.layoutConstraints);
  }
  ...
  addLayoutConstraint(
    ...
    if (floatReference === this.floatReference) {
      this.state.layoutConstraints.push(layoutConstraint);
    } else {
...
/** The only class that can be attached, which is what limits it to once. */
export class UnattachedPageFloatLayoutContext extends PageFloatLayoutContext {
  /**
   * A new context rather than a mutated one, which is what keeps a discarded
   * attempt's container out of the next one.
   */
  withContainer(container: Vtree.Container): AttachedPageFloatLayoutContext {
    return new AttachedPageFloatLayoutContext(
      this.identity(),
      container,
      this.state,
    );
...
/** A context enters the tree when it is attached; an unattached one is never a parent. */
export abstract class ParentPageFloatLayoutContext
  extends PageFloatLayoutContext
  implements PageFloats.ParentPageFloatLayoutContext
{
  get children(): readonly AttachedPageFloatLayoutContext[] {
    return this.state.children;
  }

  abstract asAttached(): AttachedPageFloatLayoutContext | null;

  attachChildren(children: AttachedPageFloatLayoutContext[]) {
  ...
  detachChildren(): AttachedPageFloatLayoutContext[] {
    const children = this.state.children.splice(0);
    children.forEach((child) => {
  ...
  collectPageFloatAnchors() {
    const anchors = Object.assign({}, this.state.floatAnchors);
    return this.state.children.reduce(
  ...
  getFloatsDeferredToNextInChildContexts(): PageFloat[] {
    ...
    for (let i = this.state.children.length - 1; i >= 0; i--) {
      ...
      result = result.concat(
        child.state.floatsDeferredToNext.map((c) => c.float),
      );
  ...
  protected findPageFloatAnchor(floatId: PageFloatID): Node | null {
    // Match collectPageFloatAnchors(): descendant registrations override
    // anchors recorded on ancestor contexts.
    for (let i = this.state.children.length - 1; i >= 0; i--) {
      const anchorViewNode =
        this.state.children[i].findPageFloatAnchor(floatId);
      if (anchorViewNode) {
        return anchorViewNode;
      }
    }
    return this.state.floatAnchors[floatId] ?? null;
  }

  protected hasRootMultiColumnFootnoteContext(): boolean {
    return this.state.children.some((child) => {
      let rootColumnCount = 0;
      for (const grandchild of child.state.children) {
  ...
  getPreviousSiblingOf(
    ...
    flowName: string | null,
    generatingNodePosition: Vtree.NodePosition | null,
  ): AttachedPageFloatLayoutContext | null {
    let index = this.state.children.indexOf(
      child as AttachedPageFloatLayoutContext,
    );
    if (index < 0) {
...
export class RootPageFloatLayoutContext extends ParentPageFloatLayoutContext {
  asAttached(): null {
    return null;
  }
  ...
  addPageFloatLayoutContextAsPreviousSibling(
    ...
    // The interface is what the viewer hands back from a rendered page; this
    // module holds the only implementation of it.
    this.state.children.push(context as AttachedPageFloatLayoutContext);
  }

  static createRoot(): RootPageFloatLayoutContext {
    return new RootPageFloatLayoutContext(
      PageFloatLayoutContext.buildIdentity(
        null,
        null,
        null,
...
export class AttachedPageFloatLayoutContext
  extends ParentPageFloatLayoutContext
  implements PageFloats.AttachedPageFloatLayoutContext
{
  asAttached(): AttachedPageFloatLayoutContext {
    return this;
  }

  /**
   * Null at the page level, whose parent is the container-less root, and in a
   * page float area, which has no parent at all.
   */
  protected readonly attachedParent: AttachedPageFloatLayoutContext | null;
  /** How a page float area's context reaches its column. (Issue #1675) */
  private readonly attachedOuter: AttachedPageFloatLayoutContext | null;
  ...
  constructor(
    ...
    this.attachedParent = identity.parent?.asAttached() ?? null;
    this.attachedOuter = identity.outerContext;
    this.reattachFloatFragments();
    // Registering here is what keeps an attachment inside its parent.
    PageFloatLayoutContext.register(this.parent, this);
  }

  protected override getParent(
    floatReference: FloatReference,
  ): AttachedPageFloatLayoutContext {
    const parent = this.attachedParent ?? this.attachedOuter;
    if (parent) {
      return parent;
    }
    throw new Error(`No PageFloatLayoutContext for ${floatReference}`);
  ...
  override getPageFloatLayoutContext(
    floatReference: FloatReference,
  ): AttachedPageFloatLayoutContext {
    if (floatReference === this.floatReference) {
      return this;
  ...
  private forbid(float: PageFloat) {
    ...
    const floatReference = float.floatReference;
    if (floatReference === this.floatReference) {
      if (!this.state.forbiddenFloats.includes(id)) {
        this.state.forbiddenFloats.push(id);
  ...
  private hasNonRootMultiColumnFootnoteContext(): boolean {
    return this.state.children.some((child) =>
      child.state.children.some((grandchild) =>
        LayoutHelper.containsNonRootMultiColumn(grandchild.container.element),
      ),
    );
  }
  ...
  initFootnoteRetryFromEmptyFragment(
    ...
    if (!(
      ...
      this.footnoteMaxBlockSize == null &&
      this.state.footnoteAnchorsSeen.has(float.getId()) &&
  ...
  checkAndForbidNotAllowedFloat(): boolean {
      ...
      if (notAllowedFloat) {
        ...
        if (this.state.locked) {
          ...
          const retryFootnoteSizing =
            isFootnote &&
            this.state.footnoteAnchorsSeen.has(notAllowedFloat.getId()) &&
    ...
    if (
      this.floatReference === FloatReference.REGION &&
      this.attachedParent?.isLocked()
    ) {
      return this.attachedParent.checkAndForbidNotAllowedFloat();
    }
    return false;
  }
  ...
  finish() {
    for (let i = this.floatFragments.length - 1; i >= 0; i--) {
      ...
      const staleLineFootnote = fragment.continuations.some((continuation) => {
        ...
        return (
          ...
          !this.state.floatsDeferredFromPrevious.some(
    ...
    for (let i = this.state.floatsDeferredToNext.length - 1; i >= 0; i--) {
      const continuation = this.state.floatsDeferredToNext[i];
      const float = continuation.float;
      ...
      if (
        ...
        this.hasCurrentAnchor(float.getId()) &&
        !existingFragment &&
        !this.state.lineFootnoteFinishInvalidatedOnce.has(float.getId())
      ) {
        ...
        this.state.lineFootnoteFinishInvalidatedOnce.add(float.getId());
    ...
    for (let i = this.state.floatsDeferredToNext.length - 1; i >= 0; i--) {
      ...
      if (!continuation.float.isAllowedOnContext(this)) {
        ...
        this.state.floatsDeferredToNext.splice(i, 1);
      }
    }
    this.state.floatsDeferredFromPrevious.forEach((continuation) => {
      if (
        this.state.floatsDeferredToNext.findIndex((c) =>
          continuation.equals(c),
        ) >= 0
      ) {
        return;
      }
  ...
  restoreStashedFragments(floatReference: FloatReference) {
    ...
    this.state.stashedFloatFragments.forEach((stashed) => {
  ...
  hasSameContainerAs(other: AttachedPageFloatLayoutContext): boolean {
    return this.container.element === other.container.element;
  }
  ...
  invalidate() {
    ...
    this.state.children.forEach((child) => {
      child.state.layoutConstraints.splice(0);
    });
    this.state.children.splice(0);
    Object.keys(this.state.floatAnchors).forEach((k) => {
      delete this.state.floatAnchors[k];
    });
  }
  ...
  private isFragmentInCurrentContainer(fragment: PageFloatFragment): boolean {
    ...
    return !area.parentElement || area.parentElement === this.container.element;
  }
    ...
    if (
      ...
      while (i < this.floatFragments.length) {
        ...
        if (
          ...
          this.state.stashedFloatFragments.push(fragment);
  ...
  private getLimitValue(
    ...
    const attachedParent = this.attachedParent;
    if (includeParent && attachedParent) {
      const parentLimit = attachedParent.getLimitValue(
        physicalSide,
        physicalSide2,
        layoutContext,
        clientLayout,
  ...
  getMaxReachedAfterEdge(): number {
    const isVertical = this.container.vertical;
  ...
  getBlockEndEdgeOfBlockStartFloats(inlinePos?: number): number {
    ...
    if (attachedParent) {
      const parentEdge =
        attachedParent.getBlockEndEdgeOfBlockStartFloats(inlinePos);
  ...
  getBlockStartEdgeOfBlockEndFloats(inlinePos?: number): number {
    ...
    if (attachedParent) {
      const parentEdge =
        attachedParent.getBlockStartEdgeOfBlockEndFloats(inlinePos);
      if (isFinite(parentEdge)) {
        edge = isFinite(edge)
          ? isVertical
            ? Math.max(edge, parentEdge)
  ...
  getPageFloatClearEdge(clear: string, column: LayoutType.Column): number {
    ...
    while (context) {
      if (
        context.state.floatsDeferredToNext.some(
          isContinuationOfAlreadyAppearedFloat(context),
        )
      ) {
    ...
    function hasFragmentWithAlreadyAppearedFloat(
      ...
      return (
        ...
        context.state.children.some((child) =>
    ...
    if (clear === "column" || clear === "region" || clear === "page") {
      for (context = this; context; context = context.attachedParent) {
  ...
  getPageFloatPlacementCondition(
    ...
    function hasPrecedingFragmentInChildren(
      ...
      return context.state.children.some(
        (child) =>
