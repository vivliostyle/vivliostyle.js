...
for (var key in rawData) {
    ...
    anchors.forEach(function(anchor) {
      if (anchor["type"] === "property") {
        ...
        anchors.sort(function(a1, a2) {
          return (
            orderedSpecs.indexOf(a1.spec.name) -
            orderedSpecs.indexOf(a2.spec.name)
          );
        });
      }
      if (anchor["children"]) {
  ...
  if (rawSpecData["anchors"])
    parseAnchors(rawSpecData["anchors"], trProperties);
  if (rawSpecData["draft_anchors"])
    parseAnchors(rawSpecData["draft_anchors"], draftProperties);
}

var rawValidations = fs.readFileSync("../resources/validation.txt", "utf8");
var propDefs = rawValidations
  .replace(/\r*\n[ \t]+/g, " ")
  .split("\n")
...
var properties = propDefs
  ...
  .sort(function(prop1, prop2) {
    ...
    var spec1Name =
      (spec1 && spec1[0] && spec1[0].spec && spec1[0].spec.name) || "";
    var spec2 = trProperties[prop2.name] || draftProperties[prop2.name];
    var spec2Name =
      (spec2 && spec2[0] && spec2[0].spec && spec2[0].spec.name) || "";
    return (
      orderedSpecs.indexOf(spec1Name) - orderedSpecs.indexOf(spec2Name) ||
...
var markdown = require("mustache").render(template, {
  properties: sectionedPropertiesView,
});
