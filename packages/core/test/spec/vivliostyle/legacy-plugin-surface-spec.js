...
// Loading the whole public entry point runs every core module's top-level
// registerHook call, so the counts below are read from a fully populated
// registry.
...
describe("legacy-plugin-surface", function () {
  ...
  var CORE_HOOK_COUNTS = vivliostyle_legacy.CORE_HOOK_COUNTS;
  var hookNames = Object.keys(CORE_HOOK_COUNTS);
  ...
  describe("CORE_HOOK_COUNTS", function () {
    it("matches the number of hooks the core registers", function () {
      hookNames.forEach(function (name) {
        expect(vivliostyle_plugin.getHooksForName(name).length).toBe(
          CORE_HOOK_COUNTS[name],
        );
      });
    });

    it("covers every hook whose payload carries a node context", function () {
  ...
  describe("noteRetained", function () {
    it("does nothing while no external hook is registered", function () {
