...
import * as vivliostyle_page from "../../../src/ts/vivliostyle/page";

describe("page", function() {
    var module = vivliostyle_page;
    var expected;
    ...
    describe("resolvePageSizeAndBleed", function() {
        ...
        it("has the same width and height when only one length is specified in size property", function() {
            var resolved = module.resolvePageSizeAndBleed({
                size: new adapt_csscasc.CascadeValue(new adapt_css.Numeric(10, "cm"), 0)
            });
        ...
        it("has the width and height specified in size property", function() {
            var resolved = module.resolvePageSizeAndBleed({
                size: new adapt_csscasc.CascadeValue(
                    new adapt_css.SpaceList([new adapt_css.Numeric(10, "cm"), new adapt_css.Numeric(12, "cm")]), 0)
            });
            expected.width = new adapt_css.Numeric(10, "cm");
        ...
        it("has the width and height of the paper size specified in size property", function() {
            ...
            resolved = module.resolvePageSizeAndBleed({
                size: new adapt_csscasc.CascadeValue(
                    new adapt_css.SpaceList([adapt_css.getName("A5"), adapt_css.getName("portrait")]), 0)
            });
            expected.width = new adapt_css.Numeric(148, "mm");
            expected.height = new adapt_css.Numeric(210, "mm");
            expect(resolved).toEqual(expected);
            ...
            resolved = module.resolvePageSizeAndBleed({
                size: new adapt_csscasc.CascadeValue(
                    new adapt_css.SpaceList([adapt_css.getName("A5"), adapt_css.ident.landscape]), 0)
            });
        ...
        it("has the default bleed offset value when 'crop' and/or 'cross' value is specified in 'marks' property", function() {
            ...
            resolved = module.resolvePageSizeAndBleed({
                ...
                marks: new adapt_csscasc.CascadeValue(
                    new adapt_css.SpaceList([adapt_css.ident.crop, adapt_css.ident.cross]), 0)
            });
            expect(resolved).toEqual(expected);
        });
        ...
        it("has the bleed value specified in 'bleed' property when it is specified with a concrete length", function() {
            var resolved = module.resolvePageSizeAndBleed({
                bleed: new adapt_csscasc.CascadeValue(new adapt_css.Numeric(5, "mm"), 0)
            });
    ...
    describe("PageParserHandler", function() {
        ...
        function createHandler() {
            return new vivliostyle_page.PageParserHandler(null, new adapt_cssparse.DispatchParserHandler(), null, null, pageProps);
        }
        ...
        it("'bleed' and 'marks' specified in @page rule without page selectors are effective", function() {
            ...
            expect(pageProps[""].size.value).toEqual(new adapt_css.Numeric(100, "px"));
            expect(pageProps[""].bleed.value).toEqual(new adapt_css.Numeric(10, "px"));
        ...
        it("'bleed' and 'marks' specified in @page rules with page selectors are ignored", function() {
            ...
            expect(pageProps[":left"].size.value).toEqual(new adapt_css.Numeric(120, "px"));
            expect(pageProps[":left"].bleed.value).toEqual(new adapt_css.Numeric(10, "px"));
            expect(pageProps[":left"].marks.value).toBe(adapt_css.ident.crop);

            expect(pageProps[":right"].size.value).toEqual(new adapt_css.Numeric(130, "px"));
            expect(pageProps[":right"].bleed.value).toEqual(new adapt_css.Numeric(10, "px"));
            expect(pageProps[":right"].marks.value).toBe(adapt_css.ident.crop);

            expect(pageProps[":first"].size.value).toEqual(new adapt_css.Numeric(140, "px"));
            expect(pageProps[":first"].bleed.value).toEqual(new adapt_css.Numeric(10, "px"));
            expect(pageProps[":first"].marks.value).toBe(adapt_css.ident.crop);

            expect(pageProps["foo"].size.value).toEqual(new adapt_css.Numeric(140, "px"));
            expect(pageProps["foo"].bleed.value).toEqual(new adapt_css.Numeric(10, "px"));
            expect(pageProps["foo"].marks.value).toBe(adapt_css.ident.crop);

            expect(pageProps["bar:first:left"].size.value).toEqual(new adapt_css.Numeric(150, "px"));
            expect(pageProps["bar:first:left"].bleed.value).toEqual(new adapt_css.Numeric(10, "px"));
            expect(pageProps["bar:first:left"].marks.value).toBe(adapt_css.ident.crop);
        });
