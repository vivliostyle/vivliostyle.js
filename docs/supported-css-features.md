...
- [CSS-wide keywords](https://www.w3.org/TR/css-values/#common-keywords): `initial`, `inherit`, `unset`, `revert`, `revert-layer`, `revert-rule`
  ...
  - When nothing is left in Vivliostyle's own cascade to roll back to, the keyword is kept as written and passed on to the browser instead of being resolved as `unset`, so the rollback lands on what Vivliostyle does not model: the browser's own user-agent style sheet and the presentational hints of attributes. This keeps the native styles of form controls such as `input` and `button`, which Vivliostyle's user-agent style sheet deliberately does not define, and lets a rollback into a shadow tree (`:host`, `::slotted`) work even though Vivliostyle's cascade does not implement Shadow DOM
  - Note: a rollback keyword reaching a property through a custom property (`--x: var(--y, revert-layer); color: var(--x)`) is not a rollback, matching browser behavior
