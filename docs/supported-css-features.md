...
- [CSS-wide keywords](https://www.w3.org/TR/css-values/#common-keywords): `initial`, `inherit`, `unset`, `revert`, `revert-layer`, `revert-rule`
  ...
  - When nothing is left in Vivliostyle's own cascade to roll back to, the keyword is passed on to the browser instead of being resolved as `unset`, so a rollback lands on the browser's user-agent style sheet. This keeps the native styles of form controls such as `input` and `button`, which Vivliostyle's user-agent style sheet deliberately does not define
