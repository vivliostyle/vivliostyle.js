...
export class Segment {
  constructor(
      public low: Point, public high: Point, public winding: number,
...
export class Band {
  ...
  constructor(
      public y1: number, public y2: number, public x1: number,
      public x2: number) {}
}
...
export const shapeForRect =
    (x1: number, y1: number, x2: number, y2: number): Shape => new Shape([
      new Point(x1, y1), new Point(x2, y1), new Point(x2, y2), new Point(x1, y2)
    ]);

export const shapeForRectObj = (r: Rect): Shape => new Shape([
  new Point(r.x1, r.y1), new Point(r.x2, r.y1), new Point(r.x2, r.y2),
  new Point(r.x1, r.y2)
]);

export class BandIntersection {
  constructor(
      public x: number, public winding: number, public shapeId: number,
...
export const addBandIntersections =
    (intersections: BandIntersection[], s: Segment, y1: number, y2: number):
        void => {
          let x1: number;
          let w1: number;
          let x2: number;
...
export const mergeIntersections =
    (intersections: BandIntersection[], includeCount: number,
     excludeCount: number): number[] => {
    ...
    (box: Rect, include: Shape[], exclude: Shape[], granularity: number,
     snapHeight: number, vertical: boolean): Band[] => {
      ...
      while (segmentIndex < segmentCount || activeSegments.length > 0) {
        ...
        // min possible y2
        const y2min =
            Math.min(ceil(Math.ceil(y + granularity), snapHeight), box.y2);
        ...
        const xranges =
            mergeIntersections(bandIntersections, includeCount, excludeCount);
        if (xranges.length == 0) {
          ...
          if (width == 0) {
            ...
            result.push(new Band(
                y, y2, Math.max(x, box.x1), Math.min(x + width, box.x2)));
          }
        }
        if (y2 == box.y2) {
          break;
...
export const findUppermostFullyOpenRect =
    (bands: Band[], rect: Rect): Rect|null => {
      if (!bands.length) {
        return rect;
      }
      let topEdge = rect.y1;
...
export const positionFloat =
    (box: Rect, bands: Band[], floatBox: Rect, side: string): boolean => {
      let y = floatBox.y1;
      ...
      while (true) {
        ...
        let x1 = box.x1;
        let x2 = box.x2;
        for (let i = index; i < bands.length && bands[i].y1 < floatBottom;
             i++) {
          const band = bands[i];
          if (band.x1 > x1) {
            x1 = band.x1;
...
export const addFloatToBands =
    (box: Rect, bands: Band[], floatBox: Rect, floatBands: Band[],
     side: string): void => {
      if (!floatBands) {
        floatBands =
            [new Band(floatBox.y1, floatBox.y2, floatBox.x1, floatBox.x2)];
      }
      ...
      for (const floatBand of floatBands) {
        ...
        if (bands[index].y1 < floatBand.y1) {
          ...
          index++;
          bands.splice(
              index, 0, new Band(floatBand.y1, band.y2, band.x1, band.x2));
        ...
        while (index < bands.length) {
          ...
          if (band.y2 > floatBand.y2) {
            // split it
            bands.splice(
                index, 0, new Band(floatBand.y2, band.y2, band.x1, band.x2));
