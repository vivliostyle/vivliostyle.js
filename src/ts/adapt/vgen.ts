...
export class ViewFactory extends base.SimpleEventTarget implements
  ...
  private static SVG_URL_ATTRIBUTES: string[] = [
    ...
    'marker-end', 'marker-mid', 'fill', 'stroke', 'mask'
  ];
  document: any;
  ...
  clone() {
    return new ViewFactory(
        this.flowName, this.context, this.viewport, this.styler, this.regionIds,
  ...
  createPseudoelementShadow(
      element: Element, isRoot: boolean, cascStyle: csscasc.ElementStyle,
      computedStyle: {[key: string]: css.Val}, styler: cssstyler.AbstractStyler,
      context: Context, parentShadow: vtree.ShadowContext,
      subShadow: vtree.ShadowContext): vtree.ShadowContext {
    const pseudoMap = this.getPseudoMap(
        cascStyle, this.regionIds, this.isFootnote, this.nodeContext, context);
    if (!pseudoMap) {
      return subShadow;
    }
    const addedNames = [];
    ...
    for (const name of pseudoNames) {
      ...
      if (name) {
        ...
        if (name === 'before' || name === 'after') {
          ...
          if (!content || content === css.ident.normal ||
  ...
  getPseudoMap(
      cascStyle: csscasc.ElementStyle, regionIds: string[], isFootnote: boolean,
      nodeContext: vtree.NodeContext, context) {
  ...
  createRefShadow(
      href: string, type, element: Element, parentShadow: vtree.ShadowContext,
      subShadow: vtree.ShadowContext): task.Result<vtree.ShadowContext> {
    const self = this;
      ...
      if (refDoc) {
        ...
        if (refElement) {
          ...
          subShadow = new vtree.ShadowContext(
              element, refElement, refDoc, parentShadow, subShadow, type,
              refStyler);
        }
      }
  ...
  createShadows(
      element: Element, isRoot, cascStyle: csscasc.ElementStyle,
      computedStyle: {[key: string]: css.Val}, styler: cssstyler.AbstractStyler,
      context: Context,
      shadowContext: vtree.ShadowContext): task.Result<vtree.ShadowContext> {
    const self = this;
    const frame: task.Frame<vtree.ShadowContext> =
    ...
    cont.then((shadow) => {
      ...
      cont2.then((shadow) => {
        shadow = self.createPseudoelementShadow(
            element, isRoot, cascStyle, computedStyle, styler, context,
   ...
   */
  computeStyle(
      vertical: boolean, rtl: boolean, style: csscasc.ElementStyle,
      computedStyle: {[key: string]: css.Val}): boolean {
    const context = this.context;
    const cascMap = csscasc.flattenCascadedStyle(
        style, context, this.regionIds, this.isFootnote, this.nodeContext);
    vertical = csscasc.isVertical(cascMap, context, vertical);
    rtl = csscasc.isRtl(cascMap, context, rtl);
    const self = this;
    csscasc.convertToPhysical(
        cascMap, computedStyle, vertical, rtl, (name, cascVal) => {
  ...
  private inheritFromSourceParent(elementStyle: csscasc.ElementStyle):
    ...
    const inheritanceVisitor =
        new csscasc.InheritanceVisitor(props, this.context);
  ...
  transferPolyfilledInheritedProps(computedStyle: {[key: string]: css.Val}) {
    const polyfilledInheritedProps =
        csscasc.getPolyfilledInheritedProps().filter(
   ...
   */
  private createElementView(firstTime: boolean, atUnforcedBreak: boolean):
      task.Result<boolean> {
    const self = this;
    ...
    self.nodeContext.vertical = self.computeStyle(
        self.nodeContext.vertical, self.nodeContext.direction === 'rtl',
    ...
    self.createShadows(
            element, isRoot, elementStyle, computedStyle, styler, self.context,
        ...
        .then((shadowParam) => {
          ...
          let floating = floatSide === css.ident.left ||
              floatSide === css.ident.right || floatSide === css.ident.top ||
          ...
          if (clearSide) {
            if (clearSide === css.ident.inherit) {
              if (self.nodeContext.parent &&
                  self.nodeContext.parent.clearSide) {
                clearSide = css.getName(self.nodeContext.parent.clearSide);
              }
            }
            if (clearSide === css.ident.left || clearSide === css.ident.right ||
                clearSide === css.ident.top || clearSide === css.ident.bottom ||
                clearSide === css.ident.both || clearSide === css.ident.all ||
          ...
          const listItem = display === css.ident.list_item &&
          ...
          self.nodeContext.verticalAlign = computedStyle['vertical-align'] &&
                  computedStyle['vertical-align'].toString() ||
              'baseline';
          self.nodeContext.captionSide = computedStyle['caption-side'] &&
          ...
          if (!borderCollapse || borderCollapse === css.getName('separate')) {
            ...
            if (borderSpacing) {
              ...
              if (inlineBorderSpacing.isNumeric()) {
                self.nodeContext.inlineBorderSpacing =
                    css.toNumber(inlineBorderSpacing, self.context);
              }
              if (blockBorderSpacing.isNumeric()) {
                self.nodeContext.blockBorderSpacing =
                    css.toNumber(blockBorderSpacing, self.context);
              }
            }
          }
          ...
          if (!self.nodeContext.inline) {
            self.processAfterIfcontinues(
                element, elementStyle, styler, self.context);
          }
          const whitespace = computedStyle['white-space'];
          if (whitespace) {
            const whitespaceValue =
                vtree.whitespaceFromPropertyValue(whitespace.toString());
          ...
          self.nodeContext.breakWord = wordBreak === css.ident.break_all ||
              overflowWrap === css.ident.break_word;

          // Resolve formatting context
          self.resolveFormattingContext(
              self.nodeContext, firstTime, display, position, floatSide,
              isRoot);
          if (self.nodeContext.parent &&
              self.nodeContext.parent.formattingContext) {
            firstTime = self.nodeContext.parent.formattingContext.isFirstTime(
                self.nodeContext, firstTime);
          }
          if (!self.nodeContext.inline) {
            self.nodeContext.repeatOnBreak =
                self.processRepeatOnBreak(computedStyle);
          ...
          if (custom) {
            ...
            elemResult = self.customRenderer(
                element, (parentNode as Element), computedStyle);
          } else {
          ...
          elemResult.then((result) => {
            if (result) {
              if (custom) {
                needToProcessChildren =
                    result.getAttribute('data-adapt-process-children') ==
                    'true';
              }
            } else {
              ...
              for (let i = 0; i < attributeCount; i++) {
                ...
                if (!attributeNS) {
                  ...
                  if (attributeName == 'id' || attributeName == 'name') {
                    ...
                    if (firstTime) {
                      attributeValue =
                          self.documentURLTransformer.transformFragment(
              ...
              if (delayedSrc) {
                ...
                if (!hasAutoWidth && !hasAutoHeight) {
                  ...
                  if (hasAutoWidth && hasAutoHeight && imageResolution &&
                      imageResolution !== 1) {
                    images.push(
                        {image, element: result, fetcher: imageFetcher});
                  }
            ...
            if (!self.nodeContext.inline) {
              let blackList = null;
              if (!firstTime) {
                if (self.nodeContext.inheritedProps['box-decoration-break'] !==
                    'clone') {
            ...
            if (listItem) {
              result.setAttribute(
                  'value', computedStyle['ua-list-item-count'].stringValue());
            }
              ...
              taskutil.waitForFetchers(fetchers).then(() => {
                if (imageResolution > 0) {
                  self.modifyElemDimensionWithImageResolution(
                      images, imageResolution, computedStyle,
  ...
  private processAfterIfcontinues(
      element: Element, cascStyle: csscasc.ElementStyle,
      styler: cssstyler.AbstractStyler, context: Context) {
    const pseudoMap = this.getPseudoMap(
        cascStyle, this.regionIds, this.isFootnote, this.nodeContext, context);
    if (!pseudoMap) {
      return;
  ...
  modifyElemDimensionWithImageResolution(
      ...
      imageResolution: number, computedStyle: {[key: string]: css.Val},
      isVertical: boolean) {
    const self = this;
          ...
          if (computedStyle['box-sizing'] === css.ident.border_box) {
            if (computedStyle['border-left-style'] !== css.ident.none) {
              scaledWidth += css.toNumber(
                  computedStyle['border-left-width'], self.context);
            }
            if (computedStyle['border-right-style'] !== css.ident.none) {
              scaledWidth += css.toNumber(
                  computedStyle['border-right-width'], self.context);
            }
            if (computedStyle['border-top-style'] !== css.ident.none) {
              scaledHeight +=
                  css.toNumber(computedStyle['border-top-width'], self.context);
            }
            if (computedStyle['border-bottom-style'] !== css.ident.none) {
              scaledHeight += css.toNumber(
                  computedStyle['border-bottom-width'], self.context);
            }
          }
  ...
  private findAndProcessRepeatingElements(
    ...
    for (let child: Node = element.firstChild; child; child = child.nextSibling) {
      ...
      this.computeStyle(
          this.nodeContext.vertical, this.nodeContext.direction === 'rtl',
          elementStyle, computedStyle);
  ...
  private preprocessTextContent(): task.Result<boolean> {
    ...
    frame
        .loop(() => {
          ...
          return hooks[index++](self.nodeContext, textContent)
   ...
   * @return holding true if children should be processed
   */
  createNodeView(firstTime: boolean, atUnforcedBreak: boolean):
      task.Result<boolean> {
    const self = this;
  ...
  isTransclusion(
      element: Element, elementStyle: csscasc.ElementStyle,
  ...
  nextInTree(nodeContext, atUnforcedBreak) {
    ...
    this.setCurrent(nodeContext, true, atUnforcedBreak)
  ...
  applyComputedStyles(
      target: Element, computedStyle: {[key: string]: css.Val}) {
    ...
    for (const propName in computedStyle) {
      ...
      if (vtree.delayedProps[propName] ||
  ...
  applyPseudoelementStyle(nodeContext, pseudoName, target) {
    ...
    nodeContext.vertical = this.computeStyle(
        nodeContext.vertical, nodeContext.direction === 'rtl', elementStyle,
        computedStyle);
    const content = computedStyle['content'];
    if (vtree.nonTrivialContent(content)) {
      content.visit(new vtree.ContentPropertyHandler(
          target, this.context, content, this.exprContentListener));
      delete computedStyle['content'];
    }
    ...
    frame
        .loop(() => {
          while (arr.length > 0) {
            pn = arr.pop();
            nodeContext =
                new vtree.NodeContext(pn.sourceNode, nodeContext, boxOffset);
            if (arr.length == 0) {
            ...
            nodeContext.shadowSibling =
                pn.shadowSibling ? pn.shadowSibling : shadowSibling;
            shadowSibling = null;
  ...
  isSameNodePositionStep(
      step1: vtree.NodePositionStep, step2: vtree.NodePositionStep): boolean {
...
export class DefaultClientLayout implements vtree.ClientLayout {
  ...
  private subtractOffsets(rect: vtree.ClientRect, originRect: vtree.ClientRect):
      vtree.ClientRect {
...
export class Viewport {
  ...
  constructor(
      public readonly window: Window, public readonly fontSize: number,
      opt_root?: HTMLElement, opt_width?: number, opt_height?: number) {
