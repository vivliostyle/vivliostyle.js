    ...
    (clientLayout: vtree.ClientLayout, element: Element,
     sizes: Size[]): {[key: Size]: number} => {
      ...
      function getComputedValue(name: string): string {
        return clientLayout.getElementComputedStyle(element).getPropertyValue(
            name);
      }
      const writingModeProperty = base.getPrefixedPropertyNames('writing-mode');
      const writingModeValue =
          (writingModeProperty ? getComputedValue(writingModeProperty[0]) :
                                 null) ||
          getComputedValue('writing-mode');
      const isVertical = writingModeValue === 'vertical-rl' ||
          writingModeValue === 'tb-rl' || writingModeValue === 'vertical-lr' ||
      ...
      const result = ({} as {[key: Size]: number});
      sizes.forEach((size) => {
        let r: string;
        switch (size) {
