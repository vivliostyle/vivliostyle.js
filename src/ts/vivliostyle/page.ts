...
export const pageSizes: {[key: string]: PageSize} = {
  ...
  'jis-b5':
      {width: new css.Numeric(182, 'mm'), height: new css.Numeric(257, 'mm')},
  'jis-b4':
      {width: new css.Numeric(257, 'mm'), height: new css.Numeric(364, 'mm')},
  'letter':
      {width: new css.Numeric(8.5, 'in'), height: new css.Numeric(11, 'in')},
  'legal':
      {width: new css.Numeric(8.5, 'in'), height: new css.Numeric(14, 'in')},
  'ledger':
      {width: new css.Numeric(11, 'in'), height: new css.Numeric(17, 'in')}
};

/**
 * Default value for line width of printer marks
 */
export const defaultPrinterMarkLineWidth: css.Numeric =
    new css.Numeric(0.24, 'pt');
 ...
 */
export const defaultPrinterMarkLineLength: css.Numeric =
    new css.Numeric(10, 'mm');
...
export const evaluatePageSizeAndBleed =
    (pageSizeAndBleed: PageSizeAndBleed,
     context: expr.Context): EvaluatedPageSizeAndBleed => {
      ...
      const bleedOffset = pageSizeAndBleed.bleedOffset.num *
      ...
      if (width === css.fullWidth) {
        if (context.pref.defaultPaperSize) {
          evaluated.pageWidth = context.pref.defaultPaperSize.width *
              context.queryUnitSize('px', false);
        } else {
          evaluated.pageWidth =
              (context.pref.spreadView ? Math.floor(context.viewportWidth / 2) -
                       context.pref.pageBorder :
                                         context.viewportWidth) -
              cropOffset * 2;
        }
      } else {
        evaluated.pageWidth =
            width.num * context.queryUnitSize(width.unit, false);
      }
      const height = pageSizeAndBleed.height;
      if (height === css.fullHeight) {
        if (context.pref.defaultPaperSize) {
          evaluated.pageHeight = context.pref.defaultPaperSize.height *
              context.queryUnitSize('px', false);
        } else {
...
export const createCornerMark =
    (doc: Document, position: CornerMarkPosition, lineWidth: number,
     cropMarkLineLength: number, bleed: number, offset: number): Element => {
      ...
      if (position === CornerMarkPosition.TOP_RIGHT ||
      ...
      if (position === CornerMarkPosition.BOTTOM_LEFT ||
          position === CornerMarkPosition.BOTTOM_RIGHT) {
        // reflect with respect to a vertical axis
...
export const createCrossMark =
    (doc: Document, position: CrossMarkPosition, lineWidth: number,
     lineLength: number, offset: number): Element => {
      const longLineLength = lineLength * 2;
      let width;
      let height;
      if (position === CrossMarkPosition.TOP ||
      ...
      if (crop) {
        Object.keys(CornerMarkPosition).forEach((key) => {
          ...
          const mark = createCornerMark(
              doc, position, lineWidth, lineLength, bleed, printerMarkOffset);
      ...
      if (cross) {
        Object.keys(CrossMarkPosition).forEach((key) => {
          ...
          const mark = createCrossMark(
              doc, position, lineWidth, lineLength, printerMarkOffset);
          container.appendChild(mark);
        });
      }
    };
    ...
    if (marginBoxesMap) {
      ...
      pageMarginBoxNames.forEach((name) => {
        if (marginBoxesMap[name]) {
          self.pageMarginBoxes[name] =
              new PageMarginBoxPartition(self.scope, self, name, style);
        }
      });
    }
  }
...
export class PageRulePartition extends pm.Partition {
  ...
  private applySpecified(style: csscasc.ElementStyle) {
    this.specified['flow-from'] =
        new csscasc.CascadeValue(css.getName('body'), 0);

    // Use absolute positioning so that this partition's margins don't collapse
    // with its parent's margins
    this.specified['position'] =
        new csscasc.CascadeValue(css.ident.absolute, 0);
...
export class PageMarginBoxPartition extends pm.Partition {
  ...
  applySpecified(style: csscasc.ElementStyle) {
    const ownStyle =
        (style[marginBoxesKey][this.marginBoxName] as csscasc.ElementStyle);
    ...
    for (let prop in style) {
      ...
      if (csscasc.inheritedProps[prop] ||
...
export class PageRuleMasterInstance extends pm.PageMasterInstance {
  ...
  adjustPageLayout(context, page, clientLayout) {
    ...
    this.sizeMarginBoxesAlongVariableDimension(
        marginBoxContainers.top, true, horizontalDimensions, context,
        clientLayout);
    this.sizeMarginBoxesAlongVariableDimension(
        marginBoxContainers.bottom, true, horizontalDimensions, context,
        clientLayout);
    this.sizeMarginBoxesAlongVariableDimension(
        marginBoxContainers.left, false, verticalDimensions, context,
        clientLayout);
    this.sizeMarginBoxesAlongVariableDimension(
        marginBoxContainers.right, false, verticalDimensions, context,
        clientLayout);
  }
  ...
  private sizeMarginBoxesAlongVariableDimension(
    ...
    const containers:
        {[key: MarginBoxPositionAlongVariableDimension]: vtree.Container} = {};
    const boxInstances: {
      [key: MarginBoxPositionAlongVariableDimension]:
  ...
  private getSizesOfMarginBoxesAlongVariableDimension(
    ...
    const sizes: {[key: MarginBoxPositionAlongVariableDimension]: number} = {};
    if (!centerBoxParam) {
        ...
        if (xOuterMaxContentSize > 0 && yOuterMaxContentSize > 0) {
          ...
          if (maxContentSizeSum < availableSize) {
            ...
            if (minContentSizeSum < availableSize) {
              result.xSize = xOuterMinContentSize +
...
class SingleBoxMarginBoxSizingParam implements MarginBoxSizingParam {
  ...
  private getSize(): {[key: sizing.Size]: number} {
    if (!this.size) {
      ...
      this.size =
          sizing.getSize(this.clientLayout, this.container.element, sizes);
    }
    return this.size;
  }
  ...
  getOuterMaxContentSize() {
    ...
    if (this.isHorizontal) {
      return this.container.getInsetLeft() +
          size[sizing.Size.MAX_CONTENT_WIDTH] + this.container.getInsetRight();
    } else {
      return this.container.getInsetTop() +
  ...
  getOuterMinContentSize() {
    ...
    if (this.isHorizontal) {
      return this.container.getInsetLeft() +
          size[sizing.Size.MIN_CONTENT_WIDTH] + this.container.getInsetRight();
    } else {
      return this.container.getInsetTop() +
  ...
  getOuterSize() {
    if (this.isHorizontal) {
      return this.container.getInsetLeft() + this.container.width +
          this.container.getInsetRight();
    } else {
      return this.container.getInsetTop() + this.container.height +
          this.container.getInsetBottom();
    }
  }
};
...
class FixedSizeMarginBoxSizingParam extends SingleBoxMarginBoxSizingParam {
  ...
  getOuterSize() {
    if (this.isHorizontal) {
      return this.container.getInsetLeft() + this.fixedSize +
          this.container.getInsetRight();
    } else {
      return this.container.getInsetTop() + this.fixedSize +
          this.container.getInsetBottom();
    }
  }
};
...
export class PageRulePartitionInstance extends pm.PartitionInstance {
  ...
  private resolvePageBoxDimensions(
    ...
    let marginStart =
        pm.toExprAuto(scope, style[`margin-${startSide}`], pageExtent);
    let marginEnd =
        pm.toExprAuto(scope, style[`margin-${endSide}`], pageExtent);
    const paddingStart =
        pm.toExprZero(scope, style[`padding-${startSide}`], pageExtent);
    const paddingEnd =
        pm.toExprZero(scope, style[`padding-${endSide}`], pageExtent);
    const borderStartWidth = pm.toExprZeroBorder(
        scope, style[`border-${startSide}-width`],
        style[`border-${startSide}-style`], pageExtent);
...
export class PageMarginBoxPartitionInstance extends pm.PartitionInstance {
  ...
  private positionAlongVariableDimension(
    ...
    const availableExtent =
        isHorizontal ? dim.borderBoxWidth : dim.borderBoxHeight;
  ...
  private positionAndSizeAlongFixedDimension(
    ...
    const extentName = names.extent;
    const pageMargin = dim[`margin${outsideName.charAt(0).toUpperCase()}${
        outsideName.substring(1)}`];
    const marginInside =
        pm.toExprZeroAuto(scope, style[`margin-${insideName}`], pageMargin);
    const marginOutside =
        pm.toExprZeroAuto(scope, style[`margin-${outsideName}`], pageMargin);
    const paddingInside =
        pm.toExprZero(scope, style[`padding-${insideName}`], pageMargin);
    const paddingOutside =
        pm.toExprZero(scope, style[`padding-${outsideName}`], pageMargin);
    const borderInsideWidth = pm.toExprZeroBorder(
        scope, style[`border-${insideName}-width`],
        style[`border-${insideName}-style`], pageMargin);
    const borderOutsideWidth = pm.toExprZeroBorder(
        scope, style[`border-${outsideName}-width`],
        style[`border-${outsideName}-style`], pageMargin);
    ...
    function getComputedValues(context: expr.Context): {
      ...
      if (result.marginInside === null || result.marginOutside === null) {
        const total = borderAndPadding + result.extent + result.marginInside +
...
export class PageManager {
  ...
  getPageRulePageMaster(
      ...
      cascadedPageStyle: csscasc.ElementStyle): pm.PageMasterInstance {
    ...
    if (!applied) {
      if (pageMaster.pseudoName === pm.userAgentPageMasterPseudo) {
        ...
        // master.
        applied =
            this.generateCascadedPageMaster(cascadedPageStyle, pageMaster);
      }
   ...
   * key.
   */
  private makeCacheKey(style: csscasc.ElementStyle, pageMaster: pm.PageMaster):
      string {
  ...
  private generatePageRuleMaster(style: csscasc.ElementStyle):
      PageRuleMasterInstance {
    const pageMaster = new PageRuleMaster(
        this.pageScope, (this.rootPageBoxInstance.pageBox as pm.RootPageBox),
        style);
    const pageMasterInstance =
        pageMaster.createInstance(this.rootPageBoxInstance);

    // Do the same initialization as in adapt.ops.StyleInstance.prototype.init
    pageMasterInstance.applyCascadeAndInit(
        this.cascadeInstance, this.docElementStyle);
  ...
  private generateCascadedPageMaster(
      style: csscasc.ElementStyle,
      pageMaster: pm.PageMaster): pm.PageMasterInstance {
    const newPageMaster =
        pageMaster.clone({pseudoName: pageRuleMasterPseudoName});
      ...
      if (marginBoxes) {
        ...
        for (const boxName in marginBoxes) {
          if (marginBoxes.hasOwnProperty(boxName)) {
            ...
            csscasc.mergeIn(
                context, targetBox, marginBoxes[boxName], specificity, null,
                null, null);
          }
        }
      }
...
export class PageParserHandler extends
  ...
  private finishSelector() {
    let selectors;
    if (!this.currentNamedPageSelector &&
  ...
  startPageMarginBoxRule(name) {
    const marginBoxMap =
        csscasc.getMutableStyleMap(this.elementStyle, marginBoxesKey);
    ...
    const handler = new PageMarginBoxParserHandler(
        this.scope, this.owner, this.validatorSet, boxStyle);
