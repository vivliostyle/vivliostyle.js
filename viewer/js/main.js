 ...
 *
 * Vivliostyle core 2019.1.100-pre.20190203062117
 */
(function(factory) {
    if (typeof define === "function" && define.amd) {
        // AMD
...
c[1]+"//gist.githubusercontent.com/"+c[2]+"/raw/"+c[6];else if(c=/^(https?:)\/\/([^/.]+\.)?jsbin\.com\/(\w+)((\/\d+)?).*$/.exec(a))a=c[1]+"//output.jsbin.com/"+c[3]+c[4]+"/";return a}b.match(/^\w{2,}:\/\/[^\/]+$/)&&(b+="/");if(a.match(/^\/\//))return(c=b.match(/^(\w{2,}:)\/\//))?c[1]+a:a;if(a.match(/^\//))return(c=b.match(/^(\w{2,}:\/\/[^\/]+)\//))?c[1]+a:a;a.match(/^\.(\/|$)/)&&(a=a.substr(1));c=b;var d=c.match(/^([^#?]*)/);b=d?d[1]:c;if(a.match(/^\#/))return b+a;c=b.lastIndexOf("/");if(0>c)return a;
for(d=b.substr(0,c+1)+a;;){c=d.indexOf("/../");if(0>=c)break;var e=d.lastIndexOf("/",c-1);if(0>=e)break;d=d.substr(0,e)+d.substr(c+3)}return d.replace(/\/(\.\/)+/g,"/")}function Ba(a){a=new RegExp("#(.*&)?"+Ea(a)+"=([^#&]*)");return(a=window.location.href.match(a))?a[2]:null}function Fa(a,b){var c=new RegExp("#(.*&)?"+Ea("f")+"=([^#&]*)"),d=a.match(c);return d?(c=d[2].length,d=d.index+d[0].length-c,a.substr(0,d)+b+a.substr(d+c)):a.match(/#/)?a+"&f="+b:a+"#f="+b}
function Ga(a){return null==a?a:a.toString()}function Ha(){this.b=[null]}Ha.prototype.length=function(){return this.b.length-1};function Ia(a,b){a&&(b="-"+b,a=a.replace(/-/g,""),"moz"===a&&(a="Moz"));return a+b.replace(/-[a-z]/g,function(a){return a.substr(1).toUpperCase()})}var Ja=" -webkit- -moz- -ms- -o- -epub-".split(" "),Ka={};
